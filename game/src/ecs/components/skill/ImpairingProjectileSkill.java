...
public abstract class ImpairingProjectileSkill implements ISkillFunction {
    ...
    /**
     * USING THE SKILL: Create a new Entity for the Forstbolt when the ability is used. this way,
     * every Frostbolt projectile acts as an Entity Create the needed Components for the Entity to
     * make it functional in game If the Frostbolt hits an enemy, the entity it collided with will
     * receive reduced movement speed (Velocity) afterward the new created projectile Entity will be
     * deleted
     */
    @Override
                            ...
                            if (comp.getCurrentManaPoints() - manaCost >= 0) {
                                ...
                                Animation animation =
                                        AnimationBuilder.buildAnimation(
                                                animationHelper(targetPoint, entity));
                                        ...
                                        (a, b, from) -> {
                                            if (b != entity) {
                                                b.getComponent(VelocityComponent.class)
                                                        .ifPresent(
                                                                evc -> { // evc --> enemy velocity
                                                                    // component
                                                                    ((VelocityComponent) evc)
                                                                            .setXVelocity(
                                                                                    projectileInfluence);
                                                                    ((VelocityComponent) evc)
                                                                            .setYVelocity(
