...
public class TeleportSkill implements ISkillFunction {
    ...
    public void execute(Entity entity) {
        ...
        entity.getComponent(ManaComponent.class).ifPresent(component -> {
            ...
            if (Game.currentLevel.getTileAt(teleportTo.toCoordinate()).isAccessible()
                && manaComp.getCurrentManaPoints() - manaCost >= 0) {

                entity.getComponent(PositionComponent.class).ifPresent(component1 -> {
                    ((PositionComponent) component1).setPosition(teleportTo);
                    manaComp.setCurrentManaPoints(manaComp.getCurrentManaPoints() - manaCost);
                });
            }
        });
    }
