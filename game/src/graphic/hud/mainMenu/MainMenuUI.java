...
/**
 * The MainMenuUI handles everything that is shown at the start. It confines the player to the
 * bounds set, thus eliminating the possibility of errors or false inputs by the player.
 *
 * <p>This class can be later modified to house logic like loading game states or even having a
 * settings menu for sounds and music.
 *
...
public class MainMenuUI<T extends Actor> extends ScreenController<T> {
    ...
    public void update() {
        ...
        if (!logo.isVisible()
                && Gdx.input.isKeyJustPressed(
                        Input.Keys.ENTER)) { // final decision in character screen
        ...
        if (logo.isVisible()) {
            if (Gdx.input.isKeyJustPressed(
                    Input.Keys.ENTER)) { // change to character screen if title screen exited
            ...
            if (Gdx.input.isKeyJustPressed(Input.Keys.DOWN)
                ...
                if (menuArrow.getX() == menuOption1.x)
                    menuArrow.setPosition(menuOption2.x, menuOption2.y);
        ...
        if (!logo.isVisible()) {
            if (Gdx.input.isKeyJustPressed(
                    Input.Keys.RIGHT)) { // iterate down in the character screen
                ...
                characterDescription.setText(characters.get(pointer).characterDescription);
                    ...
                    Input.Keys.LEFT)) { // iterate up in the character screen
                characters.get(pointer).character.setVisible(false);
                pointer--;
    ...
    private void setupMainMenu() {
        ScreenImage background = new ScreenImage("hud/main_background.png", new Point(0, 0));
        ...
        logo =
                new ScreenImage(
                        "logo/game_logo.png",
                        new Point(
                                (float) Constants.WINDOW_WIDTH / 8, Constants.WINDOW_HEIGHT - 140));
        logo.scaleBy(-1.5f);
        add((T) logo);
        logger.log(Level.FINE, "Logo loaded.");

        menuArrow = new ScreenImage("hud/arrow.png", menuOption1);
        ...
        newGame =
                new ScreenText(
                        "NEW GAME", new Point((float) Constants.WINDOW_WIDTH / 2 - 60, 140), 1f);
    ...
    private void setupCharScreen() {
        chooseCharacter =
                new ScreenText(
                        "Choose a character:",
                        new Point(
                                (float) Constants.WINDOW_WIDTH / 2 - 100,
        ...
        characterCard = new ScreenImage("hud/characterCard.png", new Point(0, 0));
