...
/**
 * A UI that allows the user to change the dungeon and visual settings.
 * This class helps outsource elements of the MainMenuUI into its own category.
...
public class SettingsMenu<T extends Actor> extends ScreenController<T> {
    ...
    /**
     * Makes all elements of this menu visible and tells its parent, that it
    ...
    public void update() {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.UP)) {
            ...
            if (pointer == 0) {
                ...
                else if (Settings.levelSize == LevelSize.LARGE) Settings.levelSize = LevelSize.MEDIUM;
                else Settings.levelSize = LevelSize.SMALL;
            }
            else if (pointer == 1) {
                Settings.setMaxMonsterAmount = Settings.setMaxMonsterAmount > Settings.MINIMUM_MONSTER_AMOUNT ? Settings.setMaxMonsterAmount - 1 : Settings.MAXIMUM_MONSTER_AMOUNT;
            }
            else if (pointer == 2) {
            ...
            if (pointer == 0) {
                ...
                else if (Settings.levelSize == LevelSize.MEDIUM) Settings.levelSize = LevelSize.LARGE;
                else Settings.levelSize = LevelSize.SMALL;
            }
            else if (pointer == 1) {
                Settings.setMaxMonsterAmount = Settings.setMaxMonsterAmount < Settings.MAXIMUM_MONSTER_AMOUNT ? Settings.setMaxMonsterAmount + 1 : Settings.MINIMUM_MONSTER_AMOUNT;
            }
            else if (pointer == 2) {
                Settings.allowDynamicLighting = !Settings.allowDynamicLighting;
            }
    ...
    private void setupSettings() {
        setting1 = new ScreenText(
            "LEVEL SIZE: " + Settings.levelSize,
            new Point(200, 250),
            1f);
        ...
        setting2 = new ScreenText(
            "MAX MONSTER: " + Settings.setMaxMonsterAmount,
            new Point(200, 200),
            1f);
        ...
        setting4 = new ScreenText(
            "POTATO MODE: " + Settings.potatoMode,
            new Point(200, 100),
            1f);
