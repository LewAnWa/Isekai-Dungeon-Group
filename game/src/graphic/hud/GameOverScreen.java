...
public class GameOverScreen<T extends Actor> extends ScreenController<T> {
    ...
    /**
     * Creates a GameOverScreen with two buttons, one for restart and the other for exiting the
     * game.
     */
    ...
    private ScreenButton buildExitButton() {
        return new ScreenButton(
                "EXIT",
                new Point((Constants.WINDOW_WIDTH) / 3f, (Constants.WINDOW_HEIGHT) / 1.5f - 40),
    ...
    private ScreenButton buildRestartButton(Game game) {
        return new ScreenButton(
                "RESTART",
                new Point(
                        (Constants.WINDOW_WIDTH) / 3f + 160, (Constants.WINDOW_HEIGHT) / 1.5f - 40),
                new TextButtonListener() {
                    @Override
                    public void clicked(InputEvent event, float x, float y) {
                        game.doRestart();
